...
void NyaDevice::Calculate(eOBJECT::GROUP group)
{
	...
	list<Bullet>::iterator it1, it_delete;

	///////////////
	// 削除処理
	///////////////
	for (it1 = attack_list_[group].begin(); it1 != attack_list_[group].end(); ++it1) {

		// 表示領域の限界を超えた
		// 他のオブジェクトと衝突するなどして弾オブジェクトのヘルスがゼロになった
		if ((int)it1->phx_->grid_x_ < 0 || 1000 < (int)it1->phx_->grid_x_ || (int)it1->phx_->grid_y_ < 0 || 700 < (int)it1->phx_->grid_y_) {
		
			it_delete = --it1;
			wait_list_.splice(wait_list_.begin(), move(attack_list_[group]), ++it_delete);

		} else if (it1->phx_->health_now_ <= 0) {

			// エフェクト描画処理
			if (it1->set_effect_.first) {
				epx.grid_x_ = (int)it1->phx_->grid_x_;
				epx.grid_y_ = (int)it1->phx_->grid_y_;
				epx.setting_id_ = it1->set_effect_.second;
//				nya_effect_->Draw(&epx);
	...
	for (auto& it2 : attack_list_[group]) {

		// 描画処理
		if (get<0>(it2.set_graphic_)) {
			gpx4.draw_angle_ = it2.draw_angle_;
			gpx4.draw_angle_ += setting_vector_[it2.device_setting_id_].draw_rotate_;
			gpx4.extend_rate_ = setting_vector_[it2.device_setting_id_].draw_extend_;
			gpx4.file_div_ = get<2>(it2.set_graphic_);
			gpx4.file_id_ = get<1>(it2.set_graphic_);
			gpx4.object_group_ = setting_vector_[it2.device_setting_id_].object_group_;
			gpx4.pos_cx_ = (int)it2.phx_->grid_x_;
			gpx4.pos_cy_ = (int)it2.phx_->grid_y_;
			nya_graphic_->Draw(&gpx4);
		}

		// 衝突判定処理
		it2.phx_->collision_pow_ = setting_vector_[it2.device_setting_id_].collision_pow_;
		it2.phx_->collision_range_ = setting_vector_[it2.device_setting_id_].collision_range_;
		nya_position_->Collision(it2.phx_, group);

		// 移動処理
		it2.phx_->grid_x_pre_ = it2.phx_->grid_x_;
		it2.phx_->grid_y_pre_ = it2.phx_->grid_y_;
		it2.phx_->grid_x_ += it2.move_x_;
		it2.phx_->grid_y_ += it2.move_y_;
	}
}
