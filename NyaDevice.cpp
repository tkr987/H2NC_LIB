...
void NyaDevice::Attack(DevicePropertyX1* dpx, GraphicPropertyX4* gadget_gpx4)
{
	...
	*it_from->dpx1_ = *dpx;
	*it_from->gadget_gpx4_ = *gadget_gpx4;
	...
	it_to = dg14_attack_list_[dpx->object_group_].begin();
	dg14_attack_list_[dpx->object_group_].splice(dg14_attack_list_[dpx->object_group_].begin(), move(dg14_wait_list_), it_from);
...
void NyaDevice::MoveGadget(eOBJECT::NUM group)
{
	...
	for (auto& it : dg14_attack_list_[group])
	{
		...
		// 描画処理
		it.gadget_gpx4_->draw_grid_cx_ = (int)it.phx_->grid_x_;
		it.gadget_gpx4_->draw_grid_cy_ = (int)it.phx_->grid_y_;
